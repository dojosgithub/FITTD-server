...
export async function loadMoreLuluLemonProducts(page, selector, categoryUrl) {
  ...
  while (true) {
    ...
    if (consecutiveNoChanges >= 7 && totalRefreshes < maxRefreshes) {
      totalRefreshes++
      ...
      // Refresh the page
      await page.goto(categoryUrl, { waitUntil: 'networkidle2' }) // <== use this instead of page.reload()
    ...
    } else if (consecutiveNoChanges >= 7 || totalRefreshes >= maxRefreshes) {
